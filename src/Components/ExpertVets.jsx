...
const VetCard = ({ vet }) => (
  <Motion.article
    className="flex flex-col items-center text-center p-4 will-change-transform"
  ...
  >
    <Motion.figure className="w-full max-w-[350px]">
      <Motion.img
        src={vet.src}
        alt={vet.alt}
        loading="lazy"
        decoding="async"
        className="w-full h-64 object-cover rounded-2xl shadow-md will-change-transform"
    ...
    <div className="mt-3 text-gray-500 space-y-2 w-full max-w-[350px]">
      {vet.lines.map((line, i) => (
        <p
          ...
          className={
            i === 0
              ? "border-b border-dashed border-orange-200 pb-3"
              : ""
          }
        >
          {line}
...
const ExpertVets = () => {
  return (
    ...
    >
      ...
      <Motion.div className="grid grid-cols-1 md:grid-cols-3 gap-10 pb-6">
        {vets.map((vet) => (
